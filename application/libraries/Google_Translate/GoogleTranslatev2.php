...
namespace GoogleTranslatev2;

use BadMethodCallException;
use ErrorException;
use GoogleTranslatev2\Tokens\GoogleTokenGenerator;
use GoogleTranslatev2\Tokens\TokenProviderInterface;
...
class GoogleTranslatev2
{
    ...
    public function __construct( $target = 'en',  $source = null, array $options = null, TokenProviderInterface $tokenProvider = null)
    {
        ...
        $this->setTokenProvider($tokenProvider == null ? new GoogleTokenGenerator : $tokenProvider)
     ...
     */
    public static function trans($string,  $target = 'en',  $source = null, array $options = [], TokenProviderInterface $tokenProvider = null)
    {
        return (new self)
            ->setTokenProvider($tokenProvider == null ? new GoogleTokenGenerator : $tokenProvider)
            ->setOptions($options) // Options are already set in client constructor tho.
            ->setSource($source)
            ->setTarget($target)
            ->translate($string);
